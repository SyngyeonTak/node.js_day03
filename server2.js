...
Post: 보내다의 의미(클라이언트가 서버에 데이터를 전송할 때 사용하는 방식)
    HTTP 프로토콜의 body를 통해 데이터를 전송하기 때문에 편지지에 데이터를 실어서 보내는 것과 같다
    , 봉토 안에 담아서 보내기 때문에 데이터가 노출되지 않는다.
...
var server = http.createServer(function(request, response){
    if(request.method=="GET"){
        ...
        request.on("data", function(param){
            // var postParam = url.parse(new String(param).toString(), true);//on메소드에서 받은 param 객체를 string형으로 변환
...
server.listen(9999, function(){
    console.log("My Server is running at port 9999...");
});